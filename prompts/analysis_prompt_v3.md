...
> **データを見て問う**:
> - 「今週の成果は GO / PIVOT / KILL のどれか？その根拠は何か？」
> - 「PIVOTする場合、変更する1変数は何か？他の変数は据え置くか？」
> - 「来週、1つだけ変えるとしたら何を『やめて』、何を『始めるべき』か？」
> - 「午前・午後・夜間の最適タスク配置は何か？」
> - 「来週の『The One Thing』（重点実験）は何にすべきか？」
> - 「想定されるリスクと、そのIf-Thenプランは何か？」

週次戦略の `success_criteria` と今週の実績を照合し、**GO / PIVOT / KILL** で今週を判定したうえで、来週の戦略を提案します。PIVOTの場合は**変更する変数を必ず1つに絞ること**が原則です。

### GO / PIVOT / KILL 判定

週次戦略の `success_criteria`（定量・定性）と今週の実績を照合し、以下の基準で今週を判定してください。

| 判定 | 条件 | アクション |
|---|---|---|
| **GO** | 成功基準を概ね達成（定量・定性ともに80%以上）| 現状維持。来週も同じ戦略・実験を継続 |
| **PIVOT** | 方向性は正しいが一部未達。根本原因が「目標の失敗」ではなく「実行設計の問題」| 目標を維持したまま、**1変数のみ**変更 |
| **KILL** | 目標自体が不適切（スコープ過大、方向性のズレ、2週連続で同じ失敗） | 目標・テーマ自体を変更 |

**1変数変更の原則（PIVOT時）**

PIVOTを選択した場合、変更するのは**1変数のみ**。複数の問題が見えても、最も根本的な1点に絞る。

- 変更しない変数: 目標、テーマ、主要タスク
- 変更する変数: 実行方法・タイミング・トリガー設計・スコープのいずれか1つ
...
【PIVOT の場合のみ】
- 変更する1変数: （具体的な行動レベルで記述）
- 変更後の仮説: （なぜその変更が効果を生むか）
- 据え置く変数: （何を変えないか）
```
...
- **Build/Pivot（来週の戦略）**: 500-800文字（GO/PIVOT/KILL判定 + The One Thing含む）
  - GO/PIVOT/KILL判定は必ず最初に記述すること
  - PIVOT時は「変更する1変数」「変更後の仮説」「据え置く変数」の3点を明記
- **全体**: スマホでスクロールして読める分量（2800-4000文字、A4用紙2-3枚相当）
...
根拠:
- 学習時間 6.8h/日（目標5h以上）: 達成
- DS平均 3.72（目標3.3以上）: 達成
- Done条件達成率 67%（目標80%）: **未達**
- 12:00タイムボックス: **未発動（2/17・2/19ともに無視）**

変更する1変数: 「12:00タイムボックスを意識的に守る」→「**12:00にアラームを設定し、強制的に次フェーズへ移行する**」（意識→構造への変換）
変更後の仮説: 意志力への依存を排除し、外部トリガー（アラーム）でタイムボックスを発動することで、設計完璧主義を構造的に防止できる
据え置く変数: 目標（rails new完了）、午前集中投下、Done条件設計、Shift日If-Thenトリガー
...
- 行動: 12:00アラーム設定（PIVOT変数）。アラームが鳴ったら即次フェーズへ移行

#### START (始めること)

**1. The One Thing（重点実験）の提案**
- **実験内容**: 12:00タイムボックスをアラームで強制発動。設計完了度に関わらず切り替える。
- **仮説**: 意識への依存（「キリの良いところまで」）を外部トリガーに置き換えることで、Done条件到達率が向上する。
- **成功の定義**: Off日の過半数（3日以上/5日中）でDone条件を時間内に達成。12:00タイムボックス発動率80%以上。
